...
export default function ProfilePage() {
  ...
  const { dark, toggleDark } = useTheme(); // ← Global dark mode
    ...
    const init = async () => {
      ...
      if (history && history.length > 0) {
        ...
        const subList = Object.entries(subMap).map(([name, v]) => ({
          name, pct: Math.round((v.correct / v.total) * 100),
    ...
    if (!notifEnabled) {
      try {
        ...
        if (permission === 'granted') {
          ...
          new Notification('ExamBuddy', { body: 'Daily reminders milashe! 🔔' });
        } else {
          alert('Browser settings thi notification allow karo.');
        }
      } catch (e) {
        alert('Notifications aa browser ma available nathi.');
      }
    } else {
  ...
  // Dark mode colors — global dark state use kare che
  const bg = dark ? '#0f0f13' : '#f0f4ff';
  const cardBg = dark ? '#1a1a24' : 'white';
  const textPrimary = dark ? '#f1f5f9' : '#1e293b';
  const textSecondary = dark ? '#94a3b8' : '#64748b';
  const borderColor = dark ? '#2d3748' : '#e2e8f0';
          ...
          <div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
                {EXAMS.map(e => (
                  <button key={e} onClick={() => saveExamTarget(e)} style={{
                    ...
                    background: examTarget === e ? (dark ? '#312e81' : '#ede9fe') : cardBg,
            ...
            <div style={{ background: cardBg, borderRadius: '16px', padding: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              ...
              {weakSubjects.length === 0 ? (
                <div style={{ textAlign: 'center', color: textSecondary, fontSize: '13px', padding: '20px' }}>Quiz ramva pachhi weakness dikhashe!</div>
              ) : weakSubjects.map((w, i) => (
                <div key={i} style={{ marginBottom: '12px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
                    <span style={{ fontSize: '13px', fontWeight: '600', color: textPrimary }}>{w.name}</span>
                    <span style={{ fontSize: '11px', fontWeight: '700', color: weakColor(w.pct), background: `${weakColor(w.pct)}20`, padding: '2px 8px', borderRadius: '6px' }}>{weakLabel(w.pct)} {w.pct}%</span>
                  </div>
                  <div style={{ background: dark ? '#2d3748' : '#f1f5f9', borderRadius: '10px', height: '6px' }}>
          ...
          <div>
            <div style={{ background: cardBg, borderRadius: '16px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
              ...
              <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Note lakhoo..." rows={3}
                style={{ width: '100%', padding: '10px', borderRadius: '10px', border: `1.5px solid ${borderColor}`, fontSize: '13px', resize: 'none', outline: 'none', fontFamily: 'Inter, system-ui', boxSizing: 'border-box', background: dark ? '#0f172a' : '#f8fafc', color: textPrimary }} />
            ...
            {notes.map(n => (
              <div key={n.id} style={{ background: cardBg, borderRadius: '14px', padding: '14px', marginBottom: '10px', boxShadow: '0 1px 4px rgba(0,0,0,0.1)', display: 'flex', gap: '10px' }}>
                <div style={{ flex: 1 }}>
        ...
        {activeTab === 'premium' && (
          <div>
            <div style={{ background: 'linear-gradient(135deg,#6366f1,#8b5cf6)', borderRadius: '20px', padding: '20px', color: 'white', marginBottom: '16px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: '18px', fontWeight: '800' }}>{isPremium ? 'Premium Active!' : 'Upgrade to Premium'}</div>
            ...
            {isPremium ? (
              <div style={{ background: cardBg, borderRadius: '16px', padding: '16px', boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                <div style={{ fontSize: '14px', fontWeight: '800', color: textPrimary, marginBottom: '12px' }}>✅ Active Features</div>
                ...
                {[
                  ...
                  <div key={i} style={{ background: cardBg, borderRadius: '16px', padding: '16px', marginBottom: '12px', border: `2px solid ${plan.btn ? plan.color : borderColor}`, position: 'relative' }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                      ...
                      {!plan.btn && <div style={{ background: dark ? '#1e293b' : '#f1f5f9', color: '#64748b', fontSize: '12px', fontWeight: '700', padding: '4px 12px', borderRadius: '10px' }}>Current</div>}
                    </div>
